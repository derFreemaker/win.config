...
---@param display lua-term.segment.parent | nil
...
function utils.display_execute(command, display)
    ...
    local execute_group = term.components.group("<execute-group>", display)
    execute_group:print("> " .. command)
    local stream = term.components.stream("execute", execute_group,
